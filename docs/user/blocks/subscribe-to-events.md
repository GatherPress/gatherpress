...
- **Events in the events archive**: the feed for a post type's archive. If your site registers more than one event post type, pick which one; leaving it on the default uses the primary event post type.
...
- **Both**: the default. Both links are listed.
