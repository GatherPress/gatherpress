...
test.describe('RSVP to an event', () => {
	...
	test('A logged in user can perform RSVP action', async ({ page }) => {
		await page.goto('/');
		await page.getByRole('link', { name: 'Events' }).click({ force: true }); // Imported from of https://github.com/GatherPress/demo-data
		...
		await page.locator('a').filter({ hasText: 'Attend' }).click({ force: true });
