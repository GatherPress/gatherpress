...
abstract class Base extends PMC_Base {
	...
	/**
	 * Normalize HTML/XML whitespace for comparison.
	 *
	 * Removes insignificant whitespace between tags while preserving
	 * whitespace within text content and attributes.
	 *
	 * @since 1.0.0
	 *
	 * @param string $html The HTML/XML string to normalize.
	 * @return string Normalized HTML/XML string.
	 */
	protected function normalize_html( string $html ): string {
	...
	/**
	 * Assert that two HTML/XML strings match, ignoring insignificant whitespace.
	 *
	 * @since 1.0.0
	 *
	 * @param string $expected Expected HTML/XML.
	 * @param string $actual   Actual HTML/XML.
	 * @param string $message  Optional failure message.
	 * @return void
	 */
	protected function assertHTMLEquals( string $expected, string $actual, string $message = '' ): void {
		$this->assertEquals(
			$this->normalize_html( $expected ),
			$this->normalize_html( $actual ),
			$message
		);
	}

	/**
	 * Assert that HTML/XML contains a substring, ignoring insignificant whitespace.
	 *
	 * @since 1.0.0
	 *
	 * @param string $needle   The substring to search for.
	 * @param string $haystack The HTML/XML to search in.
	 * @param string $message  Optional failure message.
	 * @return void
	 */
	protected function assertHTMLContains( string $needle, string $haystack, string $message = '' ): void {
		$this->assertStringContainsString(
			$this->normalize_html( $needle ),
			$this->normalize_html( $haystack ),
			$message
		);
	}
}
