...
- **After any release, two loop-closing steps are mandatory** (until #1921 automates them): merge the auto-opened `release/X.Y.Z` rollup PR into develop promptly (squash; may need an admin override because the workflow's rollup commit is unsigned), and cherry-pick that rollup onto `main` for changelog parity. An unmerged rollup PR means the next release double-rolls already-shipped changelog entries.
