		...
		while ( $gatherpress_query->have_posts() ) {
			...
			<div class="gp-past-events__container">
				...
				if ( has_post_thumbnail() ) {
					?>
					<figure class="gp-past-events__image">
						<?php the_post_thumbnail( 'medium' ); ?>
					</figure>
					<?php
				}
				?>
				<div class="gp-past-events__content">
					<h5 class="gp-past-events__datetime">
						<?php echo esc_html( $gatherpress_event->get_datetime_start() ); ?>
					</h5>
					<h3 class="gp-past-events__title">
						<a href="<?php the_permalink(); ?>" class="block">
							<?php the_title(); ?>
						</a>
					</h3>
					<div class="gp-past-events__excerpt">
						<?php the_excerpt(); ?>
					</div>
				</div>
			</div>
