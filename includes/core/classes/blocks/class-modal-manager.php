...
class Modal_Manager {
	...
	/**
	 * Attaches modal open behavior to elements with the class 'gatherpress--has-modal-trigger'.
	 *
	 * This method scans the block content for elements containing the 'gatherpress--has-modal-trigger'
	...
	public function attach_modal_open_behavior( string $block_content ): string {
		...
		// Process only tags with the specific class 'gatherpress--has-modal-trigger'.
		while ( $tag->next_tag() ) {
			$class_attr = $tag->get_attribute( 'class' );

			if ( $class_attr && false !== strpos( $class_attr, 'gatherpress--has-modal-trigger' ) ) {
				if (
	...
	/**
	 * Attaches modal close behavior to elements with the class 'gatherpress--has-modal-trigger'.
	 *
	 * This method scans the block content for elements containing the 'gatherpress--has-modal-trigger'
	 * class. If such elements are found, it applies the appropriate interactivity attributes
	...
	public function attach_modal_close_behavior( string $block_content ): string {
		...
		while ( $tag->next_tag() ) {
			...
			if ( $class_attr && false !== strpos( $class_attr, 'gatherpress--has-modal-trigger' ) && $tag->get_attribute( 'data-close-modal' ) ) {
				if (
					// @phpstan-ignore-next-line
					$tag->next_tag() &&
					in_array( $tag->get_tag(), array( 'A' ), true )
