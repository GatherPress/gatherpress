...
/**
 * Class responsible for managing cron jobs.
 *
...
class Rsvp_Cleanup
{
	...
	private function setup_hooks()
	{
		...
		add_action('gatherpress_rsvp_cleanup_cron', array($this, 'rsvp_cleanup'), 10, 0);
	...
	public function schedule_cleanup_cron()
	{
		...
		if ($switch == 'on') {
			if (!wp_next_scheduled('gatherpress_rsvp_cleanup_cron')) {
				...
				wp_schedule_single_event(time() + $time_in_seconds, 'gatherpress_rsvp_cleanup_cron');
			}
		}
	}
	...
	public function reschedule_cleanup_cron($old_value, $new_value): void
	{
		...
		if ($old_interval !== $new_interval || $old_frequency !== $new_frequency) {
			wp_clear_scheduled_hook('gatherpress_rsvp_cleanup_cron');
			$this->schedule_cleanup_cron();
		}
	}
}
