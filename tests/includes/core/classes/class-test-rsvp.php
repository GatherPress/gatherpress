...
class Test_Rsvp extends Base {
	...
	public function test_save(): void {
		...
		$this->assertSame( $status, $rsvp->save( $user->ID, $status ) );

		$status = 'not_attending';

		$this->assertSame( $status, $rsvp->save( $user->ID, $status ) );

		$this->assertEmpty( $rsvp->save( 0, $status ) );

		$status = 'unittest';

		$this->assertEmpty( $rsvp->save( $user->ID, $status ) );
	}
	...
	public function test_attending_limit_reached(): void {
		...
		$response = new Rsvp( $post->ID );

		$this->assertFalse(
			$response->attending_limit_reached( 'attending' ),
